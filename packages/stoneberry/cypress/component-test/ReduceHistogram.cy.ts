import { histogramModule } from "stoneberry/histogram-texture";
...
it("reduce 2 histograms within one src block", async () => {
  await withAsyncUsage(async () => {
    ...
    const binOp = histogramModule(histogramSize);
    const shader = new ReduceBuffer({ device, source, binOps: binOp });
    trackUse(shader);
