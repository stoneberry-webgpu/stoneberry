...
import { getHistogramPipeline } from "./ReductHistogramPipeline.js";
 ...
 */
export class ReduceHistogram extends HasReactive implements ComposableShader {
  /** Source data to be reduced */
  ...
  constructor(params: HistogramParams) {
    super();
    assignParams<ReduceHistogram>(this, params, defaults);
  }
  ...
  private encodePass(
    ...
    const label = `${this.label} histogram ${dispatchLabel}`;
   ...
   * (results are copied from the {@link ReduceHistogram.result} GPUBuffer)
   * @returns a single reduced result value in an array
   */
  async reduce(): Promise<number[]> {
    return runAndFetchResult(this, this.template, `${this.label} histogram`);
  }
  ...
  private createBuffer(size: number, dispatchLabel: string): GPUBuffer {
    const buffer = this.device.createBuffer({
      label: `${this.label} histogram ${dispatchLabel}`,
      size,
  ...
  private createBindGroup(
    ...
    return this.device.createBindGroup({
      label: `${this.label} histogramReduce ${bindLabel}`,
  ...
  @reactively private get uniforms(): GPUBuffer {
    const uniforms = this.device.createBuffer({
      label: `${this.label} histogram uniforms`,
